...
def jarvis_patrick_clustering():
    ...
    answers["spectral_function"] = jarvis_patrick
    ...
    # [i*1000:(i+1)*1000-1]
    plt.scatter(data[1000*highest_ari_index:(highest_ari_index+1)*1000, 0], data[1000*highest_ari_index:(highest_ari_index+1)*1000, 1], c=preds_final[highest_ari_index], cmap='viridis', marker='.')
    ...
    plt.scatter(data[1000*least_sse_index:(least_sse_index+1)*1000, 0], data[1000*least_sse_index:(least_sse_index+1)*1000, 1], c=preds_final[least_sse_index], cmap='viridis', marker='.')
    # plt.scatter(true_labelsv[:, 0], true_labelsv[:, 1], c=datav, cmap='viridis', marker='.')
    plt.title('Least SSE')
    plt.xlabel(f'Feature 1 for Dataset{i+1}')
    plt.ylabel(f'Feature 2 for Dataset{i+1}')
    ...
    answers["cluster scatterplot with largest ARI"] = preds_final[highest_ari_index]
    answers["cluster scatterplot with smallest SSE"] = preds_final[least_sse_index]
    ...
    # A single float
    answers["mean_ARIs"] = float(np.mean(np.array(ARI_sum)))
    # print(type(float(np.mean(np.array(ARI_sum)))))

    # A single float
    answers["std_ARIs"] = float(np.std(np.array(ARI_sum)))
    # print(type(np.std(np.array(ARI_sum))))

    # A single float
    answers["mean_SSEs"] = float(np.mean(np.array(SSE_sum)))

    # A single float
    answers["std_SSEs"] = float(np.std(np.mean(SSE_sum)))
